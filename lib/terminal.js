...
// it (→ default socket). The closetab integration tests DO set it, so the
...
// Force a UTF-8 client: tmux 3.3a otherwise replaces format tabs with underscores in the C locale.
function tmuxArgs(args) { const sock = process.env.SYSMON_TMUX_SOCKET || ''; return sock ? ['-u', '-L', sock, ...args] : ['-u', ...args]; }
